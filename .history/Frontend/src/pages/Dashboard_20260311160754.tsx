...
export default function Dashboard(){
  ...
  const createTodo = async (title: string, description: string, priority: string, dueDate: string)=>{
    if(!token) return
    try {
      await API.post("/todos/createTodo", {title, description, priority, dueDate})
  ...
  const toggleTodo = async (id:string)=>{
    ...
    if(!token) return
    try {
      await API.put(`/todos/updateTodo/${id}`, { completed: !todo.completed })
      toast.success(todo.completed ? "Marked as pending" : "Marked as completed")
      getTodos()
    } catch (error) {
  ...
  const updateTodo = async (id:string, title:string, description:string, priority:string, dueDate:string)=>{
    if(!token) return
    try {
      await API.put(`/todos/updateTodo/${id}`, { title, description, priority, dueDate })
