  ...
  return(
    <div className={`min-h-screen ${bgClass} p-6`}>
      ...
      <button
        ...
        className="fixed top-4 right-4 p-3 rounded-full bg-gray-600 text-white shadow-lg hover:bg-gray-700 transition"
      >
        {isDarkMode ? "☀️" : "🌙"}
      </button>
      ...
      <div className="max-w-3xl mx-auto px-4">
        {/* User Profile Header */}
        <div className={`${cardClass} p-4 sm:p-6 rounded-xl shadow-lg mb-6`}>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            ...
            <button
              ...
              className="bg-black text-white px-4 py-2 rounded-lg hover:bg-gray-400 transition"
            >
              Logout
            </button>
          </div>
        ...
        {/* Stats & Actions */}
        <div className={`${cardClass} p-5 rounded-xl shadow-lg mb-6`}>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-black">{todos.filter(t=>t.completed).length}</p>
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-black">{todos.filter(t=>t.priority === "high").length}</p>
          ...
          <div className="flex space-x-2">
            <button
              onClick={exportTodos}
              className="flex-1 bg-black text-white px-4 py-2 rounded-lg hover:bg-gray-400 transition text-sm"
            >
              📥 Export
            </button>
          </div>
        ...
        <div className={`${cardClass} p-4 rounded shadow-md mb-4 flex space-x-2 flex-wrap gap-2`}>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg ${filter === "pending" ? "bg-black text-white" : "bg-gray-200"}`}
          >
            Pending
          </button>
          <button
            onClick={()=>{setFilter("completed"); setCurrentPage(1)}}
            className={`px-4 py-2 rounded-lg ${filter === "completed" ? "bg-indigo-600 text-white" : "bg-gray-200"}`}
          >
            Completed
          </button>
          <button
            onClick={()=>{setFilter("high"); setCurrentPage(1)}}
            className={`px-4 py-2 rounded-lg ${filter === "high" ? "bg-red-600 text-white" : "bg-gray-200"}`}
          >
            High Priority
          </button>
        </div>
